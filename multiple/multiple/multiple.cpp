...
BEGIN_MESSAGE_MAP(CmultipleApp, CWinAppEx)
	ON_COMMAND(ID_APP_ABOUT, &CmultipleApp::OnAppAbout)
	// 基于文件的标准文档命令
...
BOOL CmultipleApp::InitInstance()
{
	...
	pDocTemplate = new CMultiDocTemplate(IDR_multipleTYPE,
		RUNTIME_CLASS(CmultipleDoc),
		RUNTIME_CLASS(CChildFrame), // 自定义 MDI 子框架
		RUNTIME_CLASS(CmultipleView));
	if (!pDocTemplate)
		return FALSE;
    ...
    //m_pMainWnd->SetWindowTextW(_T("   multiple by ZW"));
	return TRUE;
}
...
// 用于运行对话框的应用程序命令
void CmultipleApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}

// CmultipleApp 自定义加载/保存方法

void CmultipleApp::PreLoadState()
{
	BOOL bNameValid;
	CString strName;
...
void CmultipleApp::LoadCustomState()
{
}

void CmultipleApp::SaveCustomState()
{
}
